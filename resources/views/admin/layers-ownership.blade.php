...
<div class="row">
    <div class="col-md-12">
        ...
        <form id="formContent" method="POST" action="{{ url('/admin/layers/ownership/' . $layer->id) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control" type="text" name="user_id" id="user_id">
                    ...
                    <option value="{{ $item->id }}" @if ($item->id === $layer->user_id) selected @endif>{{ $item->name }}</option>
                    @endforeach
                </select>
